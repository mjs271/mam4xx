...
void srf_emissions_smoketest(Ensemble *ensemble) {
  ensemble->process([=](const Input &input, Output &output) {
    ...
    // // NOTE: vmr turns out to be unused, but still in the fxn signature for
    // now auto vmr = input.get_array("vmr"); const Real pmid =
    // input.get_array("pmid")[0]; const int nfs = input.get_array("nfs")[0];
    ...
    // mam4::mo_srf_emissions::srf_emissions_single_level(invariants, tfld,
    // h2ovmr, vmr.data(),
