...
// copy 3d arrays
KOKKOS_INLINE_FUNCTION
void assign_3d_array(const int first_dimlen,  // in
                     const int second_dimlen, // in
  ...
  for (int ifd = 0; ifd < first_dimlen; ++ifd) {
    for (int isd = 0; isd < second_dimlen; ++isd) {
      for (int itd = 0; itd < third_dimlen; ++itd) {
        const int index =
            ifd * second_dimlen * third_dimlen + isd * third_dimlen + itd;
        arr_out[index] = num;
      }
    }
  }
    ...
    Real (&qqcwsub)[gas_pcnst][maxsubarea()])              // inout
{
  ...
  for (int imode = 0; imode < num_modes; ++imode) {
    ...
    EKAT_KERNEL_ASSERT_MSG(
        fcldy != 0, "Error! set_subarea_qnumb_for_cldbrn_aerosols - fcldy is "
                    "zero\n");
    ...
    Real (&qqcwsub)[gas_pcnst][maxsubarea()]) // inout
{
  ...
  for (int imode = 0; imode < num_modes; ++imode) {
    ...
    for (int ispec = 0; ispec < mam4::num_species_mode(imode); ++ispec) {
      ...
      qqcwsub[icnst][jclea] = 0;
      EKAT_KERNEL_ASSERT_MSG(
          fcldy != 0, "Error! set_subarea_qmass_for_cldbrn_aerosols - fcldy is "
                      "zero\n");
      qqcwsub[icnst][jcldy] = qqcwgcm[icnst] / fcldy;
    } // ispec - species loop
  }   // imode - mode loop
                           ...
                           Real &factor_clea, Real &factor_cldy) // out
{
  ...
  EKAT_KERNEL_ASSERT_MSG(fcldy != 0, "Error! get_partition_factors - fcldy is "
                                     "zero\n");
